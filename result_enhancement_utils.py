...
def extract_mp4_each_consecutive_text(video_file_path, count_dict, result_df):
    enhance_video_list = []
    for i in count_dict:
        min_timestamp = result_df.iloc[count_dict[i]["index_range"][0]:count_dict[i]["index_range"][1]+1]['start'].min()
        max_timestamp = result_df.iloc[count_dict[i]["index_range"][0]:count_dict[i]["index_range"][1]+1]['end'].max()
        enhance_video_path = f'output_clip_{count_dict[i]["cleaned_key"]}.mp4'
